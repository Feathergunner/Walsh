      ...
      if (x>0 || y>0){
        ...
        for (int xx=0; xx<w_lit; xx++)
        	for (int yy=0; yy<h_lit; yy++)
          	switch(type){
          	case EXP_DISCRETE    :
          		gray[w_lit*x+xx][h_lit*y+yy] = discreteTransform( gray[xx][yy] , startMatrix[x][y] );
          		break;
            case EXP_SMOOTH_STD  :
            	gray[w_lit*x+xx][h_lit*y+yy] = Transform( gray[xx][yy] , startMatrix[x][y], WalshTransformations::smoothTransform );
            	break;
            case EXP_SMOOTH_MID  :
            	gray[w_lit*x+xx][h_lit*y+yy] = Transform( gray[xx][yy] , startMatrix[x][y], WalshTransformations::smoothMidTransform );
            	break;
            case EXP_SMOOTH_NOIZ :
            	gray[w_lit*x+xx][h_lit*y+yy] = Transform( gray[xx][yy] , startMatrix[x][y], WalshTransformations::noizeTransform );
            	break;
            case EXP_SMOOTH_NOI2 :
            	gray[w_lit*x+xx][h_lit*y+yy] = Transform( gray[xx][yy] , startMatrix[x][y], WalshTransformations::noiz2Transform );
            	break;
            case EXP_SMOOTH_NOI3 :
            	gray[w_lit*x+xx][h_lit*y+yy] = Transform( gray[xx][yy] , startMatrix[x][y], WalshTransformations::noiz3Transform );
            	break;
            }
      }
    }
